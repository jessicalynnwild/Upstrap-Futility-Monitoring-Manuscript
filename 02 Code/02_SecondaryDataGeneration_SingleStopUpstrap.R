###############################################################
### file: 02_SecondaryDataGeneration_SingleStopUpstrap      ###
### authors: Jess Wild                                      ###
### creation date:    07/20/2023                            ###
### latest edit date: 07/24/2023                            ###
### description: code to apply single stopping boundaries   ###
###############################################################
...
###############################################################
### APPLY SINGLE STOP UPSTRAPPING TO CU SIMULATION RESULTS  ###
###############################################################

#use stopping boundary results to get expected sample size and full trial result statistics for single stopping point calibrated upstrapping
dat_final_results_calibrated_0.25 <- merge(dat_setting %>% select(Setting,n,interim_fraction,power), 
  ...
  mutate(ExpectedN_FO = ifelse(stop_futility.0.25==1, yes = 0.25*n, no = n),
         ExpectedN_EO = ifelse(stop_efficacy.0.25==1, yes = 0.25*n, no = n),
         ExpectedN_FE = ifelse(stop_efficacy.0.25==1|stop_futility.0.25==1, yes = 0.25*n, no = n),
  ...
  select(n,power,Iteration,ExpectedN_FO,ExpectedN_EO,ExpectedN_FE,Decision_FO,Decision_EO,Decision_FE,
         Decision_FO_25,Decision_EO_25,Decision_FE_25,Decision_FO_50,Decision_EO_50,Decision_FE_50,
         Decision_FO_75,Decision_EO_75,Decision_FE_75,Decision_FE_F,Decision_FE_E,RejectionRate_FO_FS,
...
dat_final_results_calibrated_0.50 <- merge(dat_setting %>% select(Setting,n,interim_fraction,power), 
                                           dat_calibrated_upstraped_results %>% select(Setting,Iteration, stop_futility, stop_efficacy), 
                                           by='Setting') %>%
  select(-Setting) %>%
  mutate(interim_fraction = factor(interim_fraction)) %>%
  reshape(idvar = c("n","Iteration","power"), 
          ...
          direction = "wide") %>%
  select(n,power,Iteration,stop_futility.0.25,stop_futility.0.5,stop_futility.0.75,stop_efficacy.0.25,stop_efficacy.0.5,stop_efficacy.0.75) %>%
  mutate(ExpectedN_FO = ifelse(stop_futility.0.5==1, yes = 0.50*n, no = n),
         ExpectedN_EO = ifelse(stop_efficacy.0.5==1, yes = 0.50*n, no = n),
         ExpectedN_FE = ifelse(stop_efficacy.0.5==1|stop_futility.0.5==1, yes = 0.50*n, no = n),
         Decision_FO = ifelse(ExpectedN_FO<n,yes = 1, no = 0),
         Decision_EO = ifelse(ExpectedN_EO<n,yes = 1, no = 0),
         Decision_FE = ifelse(ExpectedN_FE<n,yes = 1, no = 0),
         Decision_FE_F = ifelse(Decision_FE==1&ExpectedN_FO<ExpectedN_EO,yes = 1, no = 0),
...
dat_final_results_calibrated_0.75 <- merge(dat_setting %>% select(Setting,n,interim_fraction,power), 
                                           dat_calibrated_upstraped_results %>% select(Setting,Iteration, stop_futility, stop_efficacy), 
                                           by='Setting') %>%
  select(-Setting) %>%
  mutate(interim_fraction = factor(interim_fraction)) %>%
  reshape(idvar = c("n","Iteration","power"), 
          ...
          direction = "wide") %>%
  select(n,power,Iteration,stop_futility.0.25,stop_futility.0.5,stop_futility.0.75,stop_efficacy.0.25,stop_efficacy.0.5,stop_efficacy.0.75) %>%
  mutate(ExpectedN_FO = ifelse(stop_futility.0.75==1, yes = 0.75*n, no = n),
         ExpectedN_EO = ifelse(stop_efficacy.0.75==1, yes = 0.75*n, no = n),
         ExpectedN_FE = ifelse(stop_efficacy.0.75==1|stop_futility.0.75==1, yes = 0.75*n, no = n),
         Decision_FO = ifelse(ExpectedN_FO<n,yes = 1, no = 0),
         Decision_EO = ifelse(ExpectedN_EO<n,yes = 1, no = 0),
         Decision_FE = ifelse(ExpectedN_FE<n,yes = 1, no = 0),
         Decision_FE_F = ifelse(Decision_FE==1&ExpectedN_FO<ExpectedN_EO,yes = 1, no = 0),
...
###############################################################
### APPLY SINGLE STOP UPSTRAPPING TO AU SIMULATION RESULTS  ###
###############################################################

#use stopping boundary results to get expected sample size and full trial result statistics for single stopping point arbitrary upstrapping
dat_final_results_arbitrary_0.25 <- merge(dat_setting %>% select(Setting,n,interim_fraction,power), 
...
dat_final_results_arbitrary_0.50 <- merge(dat_setting %>% select(Setting,n,interim_fraction,power), 
                                          dat_arbitrary_upstraped_results %>% select(Setting,Iteration, stop_futility, stop_efficacy), 
                                          by='Setting') %>%
  select(-Setting) %>%
  mutate(interim_fraction = factor(interim_fraction)) %>%
...
dat_final_results_arbitrary_0.75 <- merge(dat_setting %>% select(Setting,n,interim_fraction,power), 
                                          dat_arbitrary_upstraped_results %>% select(Setting,Iteration, stop_futility, stop_efficacy), 
                                          by='Setting') %>%
  select(-Setting) %>%
  mutate(interim_fraction = factor(interim_fraction)) %>%
...
###############################################################
### APPLY SINGLE STOP UPSTRAPPING TO GU SIMULATION RESULTS  ###
###############################################################

#use stopping boundary results to get expected sample size and full trial result statistics for single stopping point upstrapping with alpha beta spending
dat_final_results_AB_0.25 <- merge(dat_setting %>% select(Setting,n,interim_fraction,power), 
  ...
  select(n,power,MonitoringType,Iteration,ExpectedN_FO,ExpectedN_EO,ExpectedN_FE,Decision_FO,
         Decision_EO,Decision_FE,Decision_FE_F,Decision_FE_E,Decision_FO_25,Decision_EO_25,
         Decision_FE_25,Decision_FO_50,Decision_EO_50,Decision_FE_50,Decision_FO_75,Decision_EO_75,
         Decision_FE_75,RejectionRate_FO_FS,RejectionRate_EO_FS,RejectionRate_FE_FS) %>%
...
dat_final_results_AB_0.50 <- merge(dat_setting %>% select(Setting,n,interim_fraction,power), 
                                   dat_calibrated_AB_results %>% select(Setting,Iteration,MonitoringType,stop_futility,stop_efficacy), 
                                   by='Setting') %>%
  select(-Setting) %>%
  mutate(interim_fraction = factor(interim_fraction)) %>%
...
dat_final_results_AB_0.75 <- merge(dat_setting %>% select(Setting,n,interim_fraction,power), 
                                   dat_calibrated_AB_results %>% select(Setting,Iteration,MonitoringType,stop_futility,stop_efficacy), 
                                   by='Setting') %>%
  select(-Setting) %>%
  mutate(interim_fraction = factor(interim_fraction)) %>%
...
save(dat_final_results_calibrated_0.25, file = './01 Data/Data Processed/dat_final_results_calibrated_0.25.RData')
#load('./01 Data/Data Processed/dat_final_results_calibrated_0.25.RData')
save(dat_final_results_calibrated_0.50, file = './01 Data/Data Processed/dat_final_results_calibrated_0.50.RData')
#load('./01 Data/Data Processed/dat_final_results_calibrated_0.50.RData')
save(dat_final_results_calibrated_0.75, file = './01 Data/Data Processed/dat_final_results_calibrated_0.75.RData')
#load('./01 Data/Data Processed/dat_final_results_calibrated_0.75.RData')
save(dat_final_results_arbitrary_0.25, file = './01 Data/Data Processed/dat_final_results_arbitrary_0.25.RData')
#load('./01 Data/Data Processed/dat_final_results_arbitrary_0.25.RData')
save(dat_final_results_arbitrary_0.50, file = './01 Data/Data Processed/dat_final_results_arbitrary_0.50.RData')
#load('./01 Data/Data Processed/dat_final_results_arbitrary_0.50.RData')
save(dat_final_results_arbitrary_0.75, file = './01 Data/Data Processed/dat_final_results_arbitrary_0.75.RData')
#load('./01 Data/Data Processed/dat_final_results_arbitrary_0.75.RData')
save(dat_final_results_AB_0.25, file = './01 Data/Data Processed/dat_final_results_AB_0.25.RData')
#load('./01 Data/Data Processed/dat_final_results_AB_0.25.RData')
save(dat_final_results_AB_0.50, file = './01 Data/Data Processed/dat_final_results_AB_0.50.RData')
#load('./01 Data/Data Processed/dat_final_results_AB_0.50.RData')
save(dat_final_results_AB_0.75, file = './01 Data/Data Processed/dat_final_results_AB_0.75.RData')
